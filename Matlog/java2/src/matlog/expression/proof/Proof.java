...
public class Proof {
    ...
    private Map<Expression, ArrayList<Expression>> rightPartMP = new HashMap<>();
...
//        Set<Expression> expressionsFromRightPartMP = rightPartMP.get(expression);
        List<Expression> expressionsFromRightPartMP = rightPartMP.get(expression);
    ...
    private boolean addExpression(ExpressionShell expressionShell) {
        ...
        if (expressionShell.getExpression().getType() == 1) {
            rightPartMP.putIfAbsent(expressionShell.getExpression().getRightSon(), new ArrayList<>());
//            rightPartMP.putIfAbsent(expressionShell.getExpression().getRightSon(), new HashSet<>());
            rightPartMP.get(expressionShell.getExpression().getRightSon()).add(0, expressionShell.getExpression());
