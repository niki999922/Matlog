...
happyExpList = Happy_Data_Array.listArray (0,34) ([6272,3136,128,128,0,0,98,49,32768,8193,4102,3,2,25088,0,0,512,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","Expr","Disj","Conj","Term","IDENT","IMPL","OR","AND","NOT","LEFTP","RIGHTP","%eof"]
        bit_start = st * 15
        bit_end = (st + 1) * 15
...
happyReduction_2 (HappyAbsSyn4  happy_var_3)
	...
	(HappyAbsSyn5  happy_var_1)
	 =  HappyAbsSyn4
		 (Binary Impl happy_var_1 happy_var_3
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	Ident happy_dollar_dollar -> cont 8;
	ImplT -> cont 9;
	OrT -> cont 10;
	AndT -> cont 11;
	NotT -> cont 12;
...
happyError' = (\(tokens, _) -> fail "Parse error" tokens)
